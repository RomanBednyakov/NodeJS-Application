...
function selectPost(req, res) {
    sequelize.query(`SELECT users.name, posts.content, posts.date, posts,title FROM users RIGHT JOIN posts ON users.id='${req.body.userId}' WHERE posts.userid='${req.body.userId}'`,{type: sequelize.QueryTypes.SELECT})
        .then((posts) => {
            posts.forEach((item) => {
                delete item.posts;
            });
